...
from vlex_ria.core.config import RLConfig, VisualizationConfig, get_device
...
class RuleBasedReward(RewardFunction):
    ...
    def __init__(
        ...
        self.legal_keywords = [
            ...
            "căn cứ", "quy định", "pháp luật", "bộ luật", "theo"
        ]
    ...
    def __call__(self, response: str, prompt: str = "", context: str = "") -> float:
        ...
        # Legal vocabulary reward (Domain specific)
        lower_response = response.lower()
        legal_matches = sum(1 for kw in self.legal_keywords if kw in lower_response)
        if legal_matches > 0:
            # Reward up to 3 different keywords to prevent just spamming one keyword
            reward += self.legal_weight * min(1.0, legal_matches / 3.0)
            
        # Phase 3: Citation-Aware & Hallucination Detection
        import re
        patterns = [r"điều\s+\d+", r"khoản\s+\d+", r"luật\s+[a-z\s]+"]
        has_citations = False
        hallucination = False
        
        for pattern in patterns:
            found_citations = re.findall(pattern, lower_response)
            if found_citations:
                has_citations = True
                lower_context = context.lower() if context else ""
                for cit in found_citations:
                    if cit not in lower_context:
                        hallucination = True
                        break
        ...
        if hallucination:
            ...
            reward += 1.0  # Citation verifiable reward
...
class BaseRLTrainer:
    ...
    def __init__(
        ...
        if ref_model is not None:
            ...
            try:
                ...
                self.ref_model = VLexRIAModel(model.config).to(self.device)
    ...
    def compute_log_probs(
        ...
        # Replace -100 with 0 to prevent out-of-bounds gather
