...
from vlex_ria.core.config import (
    VLexRIAConfig, TrainingConfig, SFTConfig, RLConfig,
    VisualizationConfig, get_device,
)
from vlex_ria.training.training_utils import (
...
class Visualizer:
    ...
    def log_moe(self, model: VLexRIAModel, step: int):
        """
    ...
    # Subset of parameter name patterns worth logging full histograms for.
    # Logging ALL params copies the entire model to CPU every call (~400MB+).
    _HISTOGRAM_PATTERNS = (
        "embed_tokens.weight",
        "lm_head.weight",
        "layers.0.attention.q_down_proj.weight",
        "layers.0.attention.kv_down_proj.weight",
        "layers.0.ffn.gate.gate.weight",     # MoE router (if present)
        "norm.weight",
    )

    def _should_log_histogram(self, name: str) -> bool:
        """Check if a parameter name matches the histogram-worthy subset."""
        return any(pattern in name for pattern in self._HISTOGRAM_PATTERNS)

    def log_gradients(self, model: VLexRIAModel, step: int):
        """Log gradient statistics.
        
        Logs scalar norms for ALL parameters (cheap — no CPU copy) and
        full histograms only for a small representative subset to avoid
        copying the entire model to CPU every call.
        """
        ...
        for name, param in model.named_parameters():
            if param.grad is not None:
                # Scalar norm is cheap: computed on-device, only a float is transferred
                self.writer.add_scalar(f"gradients/{name}_norm", param.grad.norm().item(), step)
                
                # Full histogram only for key layers and at reduced frequency
                if (step % (self.config.gradient_log_steps * 5) == 0
                        and self._should_log_histogram(name)):
                    self.writer.add_histogram(f"gradients/{name}", param.grad.cpu(), step)
    
    def log_weights(self, model: VLexRIAModel, step: int):
        """Log weight distributions.
        
        Logs full histograms only for a small representative subset of
        parameters to avoid copying the entire model to CPU every call.
        """
        if not self.config.visualize_weights:
            return
        
        for name, param in model.named_parameters():
            if self._should_log_histogram(name):
...
class BaseTrainer:
                ...
                if num_batches % self.config.gradient_accumulation_steps == 0:
                    ...
                    if self.global_step % self.config.eval_steps == 0:
                        ...
                        if val_loss < self.best_val_loss:
                            ...
                            logger.info(f"[SUCCESS] New best model saved! (val_loss: {val_loss:.4f})")
    ...
    def _training_step_with_metrics(self, batch: Dict[str, torch.Tensor]) -> Tuple[torch.Tensor, Dict[str, float]]:
        ...
        """
        dtype_map = {"bf16": torch.bfloat16, "fp16": torch.float16, "no": None}
        amp_dtype = dtype_map.get(getattr(self.config, 'mixed_precision', 'no'), None)
        from contextlib import nullcontext
        ctx = nullcontext() if amp_dtype is None else torch.autocast(device_type=self.device.type, dtype=amp_dtype)
...
class SFTTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        if getattr(config, 'use_lora', False):
            ...
            logger.info("Initializing PEFT LoRA model for SFT...")
            lora_config = LoraConfig(
                task_type=TaskType.CAUSAL_LM,
                r=getattr(config, 'lora_r', 16),
                lora_alpha=getattr(config, 'lora_alpha', 32),
                lora_dropout=getattr(config, 'lora_dropout', 0.05),
                target_modules=["q_down_proj", "q_up_proj", "kv_down_proj", "k_up_proj", "v_up_proj", "o_proj"]
            )
...
def create_trainer(
    ...
    tokenizer: Any,
    mode: str = "pretrain",
    ref_model: Optional[VLexRIAModel] = None,
) -> BaseTrainer:
    """
