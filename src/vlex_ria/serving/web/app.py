...
# Project root for finding configs/checkpoints (not for imports)
...
from vlex_ria.core.config import get_device, load_config
...
def list_checkpoints(checkpoints_dir: str = "artifacts/checkpoints") -> List[CheckpointInfo]:
    """
...
def load_model(checkpoint_path: str) -> Dict[str, Any]:
    ...
    with _model_lock:
        ...
        # Load config (prefer config.yaml)
        ...
        # Load checkpoint (needs weights_only=False because checkpoints contain config objects)
...
if __name__ == '__main__':
    logger.info("Starting VLexRIA Web Chat Interface")
    ...
    if checkpoints:
        ...
        logger.warning("No checkpoints found in artifacts/checkpoints directory")
